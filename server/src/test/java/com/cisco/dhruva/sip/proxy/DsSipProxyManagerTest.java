...
public class DsSipProxyManagerTest {
    ...
    public void testRequestInterface() throws DsException {
        ...
        when(controllerFactory.getController(any(DsSipServerTransaction.class),
                any(DsSipRequest.class),
                any(ProxyAdaptorFactoryInterface.class),
                any(AppInterface.class),
                any(DsProxyFactoryInterface.class))).thenReturn(controller);
    ...
    public void testStrayResponseViaWithoutBranch() throws DsException, IOException {
        ...
        DsSipViaHeader localViaHeader =  new DsSipViaHeader(new DsByteString("127.0.0.1"), 5060, Transport.UDP);
    ...
    public void testStrayResponse() throws DsException, IOException {
        ...
        DsSipViaHeader localViaHeader1 =  new DsSipViaHeader(new DsByteString("127.0.0.1"), 5070, Transport.UDP);
        localViaHeader1.setBranch(new DsByteString("z9hG4bKUsaQangfWbsEVmsoPdTNBA~~4"));
        response.addHeader(localViaHeader1, true, false);

        DsSipViaHeader localViaHeader2 =  new DsSipViaHeader(new DsByteString("127.0.0.1"), 5060, Transport.UDP);
        ...
        doReturn(true).when(transportLayer).isListenInterface(anyString(), anyInt(), any(Transport.class));
        when(transportLayer.getConnection(any(DsNetwork.class), any(InetAddress.class), anyInt(), any(Transport.class))).thenReturn(sendConnection);
        dsSipProxyManager.strayResponse(response);
