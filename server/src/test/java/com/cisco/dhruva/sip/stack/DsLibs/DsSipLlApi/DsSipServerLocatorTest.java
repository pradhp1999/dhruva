...
public class DsSipServerLocatorTest {
    protected static final Logger logger = DhruvaLoggerFactory.getLogger(DsSipServerLocatorTest.class);

    // Configure immediate eviction of {SRV, A} records in cache. Per Guava's CacheBuilder documentation, if maximum
    ...
    public void testAugmentRealDNSRecords(Method m) throws ExecutionException, InterruptedException {
        ...
        mockDnsLookup.addSRVRecord("_sips._tcp.dhruva.com", new DNSSRVRecord("_sips._tcp.dhruva.com", 1800L, 1000, 10, 25061, host1 + "."));
        mockDnsLookup.addSRVRecord("_sips._tcp.dhruva.com", new DNSSRVRecord("_sips._tcp.dhruva.com", 1800L, 1000, 10, 25061, host2 + "."));
        mockDnsLookup.addARecord(host1 + ".", new DNSARecord("dhruva-dhruvaintb-01.ciscospark.com.", 5, hostIP));
        mockDnsLookup.addARecord(host2 + ".", new DNSARecord("dhruva-dhruvaintb-02.ciscospark.com.", 5, hostIP));

        Set<Hop> realHops = new HashSet<>(Arrays.asList(new Hop(host1, hostIP, Transport.TLS, port,  1000, DNSRecordSource.DNS),
        ...
        try {
            ...
            dnsInjectionService.injectSRV(userIdInject, Arrays.asList(new InjectedDNSSRVRecord(injectSRVName, 3601, 100, 1000, injectPort5065, injectDnsHostname, DNSInjectAction.PREPEND)));
            dnsInjectionService.injectA(userIdInject, Arrays.asList(new InjectedDNSARecord(injectDnsHostname, 3600, injectAddress, DNSInjectAction.REPLACE)));
            LocateSIPServersResponse response = locator.resolve(name.toLowerCase(), eTransport, null, userIdInject);
            ...
            Assert.assertEquals(response.getHops().get(0), new Hop(injectHostname, injectAddress, Transport.TLS, injectPort5065, 100, DNSRecordSource.INJECTED));
            ...
            try {
                ...
                logger.error("Exception trapped in finally block " + m.getName() + ": exception=", finallyException);
            }
        }
    }
    ...
    public void testSipServerLocatorLookupTypes(Method m) throws Exception {
        ...
        // Verify paths through the locator logic depending on type of search and whether SRV or A records are found.
        // incorporate userIdInject into names makes easier to grep traces
        final String userIdInject = UUID.randomUUID().toString().toLowerCase();
        final String name = "bogus-" + UUID.randomUUID().toString() + "-dhruva-test.com";   // domain name for both sip and sips.
        ...
        final String injectSRVTarget = srvHostname + ".";  // Same as above with period, formatted for SRV record
        final String injectSRVTargetAddress = "1.2.3.4";   // Address to which the name above will resolve
        final String injectAddress = "2.3.4.5"; // Address to which "name" will resolve if looked up directly as host.
        ...
        try {
            ...
            // The address is different from above, which should allow us to deduce which path was followed.
            dnsInjectionService.injectSRV(userIdInject, Arrays.asList(
                    new InjectedDNSSRVRecord(injectSRVTLSName, 3601, 100, 1000, 5065, injectSRVTarget, DNSInjectAction.REPLACE),
                    new InjectedDNSSRVRecord(injectSRVTCPName, 3601, 100, 1000, 5060, injectSRVTarget, DNSInjectAction.REPLACE),
                    new InjectedDNSSRVRecord(injectSRVTCPNameOnly, 3601, 100, 1000, 5060, injectSRVTarget, DNSInjectAction.REPLACE)));
            dnsInjectionService.injectA(userIdInject, Arrays.asList(
                    new InjectedDNSARecord(injectSRVTarget, 3600, injectSRVTargetAddress, DNSInjectAction.REPLACE),
                    new InjectedDNSARecord(name + ".", 3600, injectAddress, DNSInjectAction.REPLACE)));

            // Test resolving as concrete transport, TLS or TCP.
            {
                logger.info("test: transport=TLS, no port specified -> RFC 3263 logic: try SRV, if no hits resort to hostname.");
                LocateSIPServersResponse response = locator.resolve(name,
                        LocateSIPServerTransportType.TLS,
                        null,
                        userIdInject);
                logger.info("response = \n{}\n", JsonUtil.toJsonPretty(response));
                Assert.assertEquals(response.getHops().size(), 1, "number of hops mismatch");
                Assert.assertEquals(response.getHops().get(0), new Hop(srvHostname, injectSRVTargetAddress, Transport.TLS, 5065, 100, DNSRecordSource.INJECTED));
            }
            {
                // Same as above, just query for TCP
                logger.info("test: transport=TCP, no port specified -> RFC 3263 logic: try SRV, if no hits resort to hostname.");
                LocateSIPServersResponse response = locator.resolve(name,
                ...
                logger.info("response = \n{}\n", JsonUtil.toJsonPretty(response));
                Assert.assertEquals(response.getHops().size(), 1, "number of hops mismatch");
                Assert.assertEquals(response.getHops().get(0), new Hop(srvHostname, injectSRVTargetAddress, Transport.TCP, 5060, 100, DNSRecordSource.INJECTED));
            }
            {
                // If we request a ANY lookup on the same hostname that is referenced by the SRV record,
                // the SRV lookup should fail and it should fallback to trying hostname, so we'll still succeed.
                // Since we pass port=null, it should calculate the default port for the protocol (TLS->5061)
                logger.info("test: ANY lookup where SRV fails but hostname lookup succeeds");
                LocateSIPServersResponse response = locator.resolve(srvHostname,
                        LocateSIPServerTransportType.TLS,
                        null,
                        userIdInject);
                logger.info("response = \n{}\n", JsonUtil.toJsonPretty(response));
                Assert.assertEquals(response.getHops().size(), 1, "number of hops mismatch");
                Assert.assertEquals(response.getHops().get(0), new Hop(srvHostname, injectSRVTargetAddress, Transport.TLS, 5061, 0,DNSRecordSource.INJECTED));
            }
            ...
            // Test resolving TLS_AND_TCP
            {
                ...
                LocateSIPServersResponse response = locator.resolve(name,
                        LocateSIPServerTransportType.TLS_AND_TCP,
                        null,
                        userIdInject);
                logger.info("response = \n{}\n", JsonUtil.toJsonPretty(response));
                Assert.assertEquals(response.getHops().size(), 1, "number of hops mismatch");
            ...
            {
                logger.info("test: transportLookupType=TLS_AND_TCP pass TCP-only SRV domain name, hit on TCP SRV");
                // This should fail the SRV TLS lookup, then hit on the SRV TCP lookup and return those hops.
                LocateSIPServersResponse response = locator.resolve(nameTCP,
                        LocateSIPServerTransportType.TLS_AND_TCP,
                        null,
                        userIdInject);
                logger.info("response = \n{}\n", JsonUtil.toJsonPretty(response));
            ...
            {
                logger.info("test: transportLookupType=TLS_AND_TCP pass hostname referenced by the SRV record");
                ...
                logger.info("response = \n{}\n", JsonUtil.toJsonPretty(response));
                Assert.assertEquals(response.getHops().size(), 1, "number of hops mismatch");
                Assert.assertEquals(response.getHops().get(0), new Hop(srvHostname, injectSRVTargetAddress, Transport.TCP, 5060, 0,DNSRecordSource.INJECTED));
            }

            {
                logger.info("test: transportLookupType=TLS_AND_TCP pass SRV domain name and TLS port, hit injectAddress");
                ...
                logger.info("response = \n{}\n", JsonUtil.toJsonPretty(response));
                Assert.assertEquals(response.getHops().size(), 1, "number of hops mismatch");
                Assert.assertEquals(response.getHops().get(0), new Hop(name, injectAddress, Transport.TLS, 5075,0, DNSRecordSource.INJECTED));
            }
            {
                logger.info("test: transportLookupType=TLS_AND_TCP pass SRV domain name and TCP port, hit injectAddress");
                ...
                logger.info("response = \n{}\n", JsonUtil.toJsonPretty(response));
                Assert.assertEquals(response.getHops().size(), 1, "number of hops mismatch");
                Assert.assertEquals(response.getHops().get(0), new Hop(name, injectAddress, Transport.TCP, 5060,0, DNSRecordSource.INJECTED));
            }

            // Test name is IPv4 address
            {
                logger.info("test: name is IP, transport=TCP, no port specified, derive default port from transport.");
                LocateSIPServersResponse response = locator.resolve("10.9.8.7",
                        LocateSIPServerTransportType.TCP,
                        null,
                        userIdInject);
                logger.info("response = \n{}\n", JsonUtil.toJsonPretty(response));
                Assert.assertEquals(response.getHops().size(), 1, "number of hops mismatch");
                Assert.assertEquals(response.getHops().get(0), new Hop(null, "10.9.8.7", Transport.TCP, 5060, 0,DNSRecordSource.DNS));
            }
            {
                logger.info("test: name is IP, transport=TLS, no port specified, derive default port from transport.");
                LocateSIPServersResponse response = locator.resolve("10.9.8.7",
                        LocateSIPServerTransportType.TLS,
                        null,
                        userIdInject);
                logger.info("response = \n{}\n", JsonUtil.toJsonPretty(response));
                Assert.assertEquals(response.getHops().size(), 1, "number of hops mismatch");
                Assert.assertEquals(response.getHops().get(0), new Hop(null, "10.9.8.7", Transport.TLS, 5061, 0,DNSRecordSource.DNS));
            }
            {
            ...
            }
            {
                ...
                logger.info("response = \n{}\n", JsonUtil.toJsonPretty(response));
                Assert.assertEquals(response.getHops().size(), 1, "number of hops mismatch");
                Assert.assertEquals(response.getHops().get(0), new Hop(null, "10.9.8.7", Transport.TLS, 5060, 0,DNSRecordSource.DNS));
            }
            {
            ...
            {
                logger.info("test: name is IP, transport=TLS_AND_TCP, port=5062 (non-standard), infer transport=TLS");
                LocateSIPServersResponse response = locator.resolve("10.9.8.7",
                        LocateSIPServerTransportType.TLS_AND_TCP,
                        5062,
                        userIdInject);
                logger.info("response = \n{}\n", JsonUtil.toJsonPretty(response));
                Assert.assertEquals(response.getHops().size(), 1, "number of hops mismatch");
                Assert.assertEquals(response.getHops().get(0), new Hop(null, "10.9.8.7", Transport.TLS, 5062, 0,DNSRecordSource.DNS));
            }
    ...
    private static class MockDnsLookup implements DnsLookup {
        ...
        public CompletableFuture<List<DNSSRVRecord>> lookupSRV(String lookup) {
            CompletableFuture<List<DNSSRVRecord>> records =
        ...
        public CompletableFuture<List<DNSARecord>> lookupA(String lookup) {
            CompletableFuture<List<DNSARecord>> records =
                    new CompletableFuture<>();
