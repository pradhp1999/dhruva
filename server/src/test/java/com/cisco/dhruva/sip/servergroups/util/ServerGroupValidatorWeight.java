...
public class ServerGroupValidatorWeight implements TestValidator {
  ...
  public ServerGroupValidatorWeight(ServerGroupInput testCase) {
  ...
  public void validate() {
    try {
      ...
      testRunInfo
          ...
          .forEach(
              element -> {
                int calls = element.getValue();
                int minimumCalls = (int) validationInfo.get(element.getKey());
                assertTrue(
                    calls >= minimumCalls, "minimum calls not received by ip" + element.getKey());
              });
  ...
  private void getValidationInfo() {
    ...
    int callDistribution = ((upElements.size() != 0) ? (totalCalls / upElements.size()) : 0);
    int minimumCalls;
    ...
    for (Elements element : elements) {
      ...
      callDistribution = (int) (((float) element.getWeight() / (float) totalWeight) * totalCalls);
      minimumCalls = (callDistribution * callVariationPercentage) / 100;
      if (isUpElement(testCase, element) && element.getQvalue() == highestQ) {
