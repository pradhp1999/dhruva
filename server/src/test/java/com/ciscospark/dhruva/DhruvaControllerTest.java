...
public class DhruvaControllerTest {
   ...
   */
  @MockBean ConfigProperties configProperties;

  @MockBean CiscoSparkServerProperties serverProperties;
  ...
  private final Greeting greeting =
      Greeting.builder().greeting("Hello spark").message("A special message for you.").build();
  ...
  public void testPostGreeting() throws Exception {
    ...
    mvc.perform(
            post("/api/v1/greetings/spark")
                .contentType(MediaType.APPLICATION_JSON)
                .content(ObjectMappers.toJson(greeting)))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.greeting").value("Hello spark"))
        .andExpect(jsonPath("$.message").value("A special message for you."));
  }
  ...
  public void testDeleteGreeting() throws Exception {
    mvc.perform(delete("/api/v1/greetings/spark")).andExpect(status().isOk());
  ...
  public void testDeleteNonExistentGreetingReturnsNotFound() throws Exception {
    Mockito.doThrow(ServerException.notFound("Greeting not found!"))
        .when(greetingService)
        .deleteGreeting(any(), any());

    mvc.perform(delete("/api/v1/greetings/spark")).andExpect(status().isNotFound());
