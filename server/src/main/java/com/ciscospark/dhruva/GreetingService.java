...
public class GreetingService {
  ...
  public GreetingService(
      DhruvaProperties properties,
      @Qualifier("store") Map<String, String> store,
  ...
  Greeting getGreeting(String name, Optional<AuthInfo> authInfo) {
    ...
    if (authInfo.isPresent()) {
      log.info(
          "Getting greeting for name : {}, userId : {}, orgId : {}",
          name,
          authInfo.get().getEffectiveUserId(),
          authInfo.get().getOrgId());
      log.info(
          LogMarker.LONGTERM.get(),
          "Log with Marker - Getting greeting for name : {}, userId : {}, orgId : {}",
          name,
          authInfo.get().getEffectiveUserId(),
          authInfo.get().getOrgId());
      log.debug(
          "DEBUG log - Getting greeting for name : {}, userId : {}, orgId : {}, isProxyUser: {}, clientId: {}",
          name,
          authInfo.get().getEffectiveUserId(),
          authInfo.get().getOrgId(),
          authInfo.get().isProxyingUser(),
          authInfo.get().getClientId());
    }
    try {
  ...
  private Greeting getEnhancedGreeting(String name, Optional<AuthInfo> authInfo) {
    ...
    //        /* Use the feature client to determine what the exact message is to return to the
    // caller.
    //         * This is simply an example of how to call another service. Note that we could have
    // simply
    //         * used an "orElse(message)" instead of "orElseGet(...)", but I wanted to include a
    // debug
    ...
    // featureClientFactory.newClient(ai.getAuthorization());
    //                    String key = serverProperties.getName() + "-adduserresponse";
    //                    FeatureToggle feature =
    // client.getDeveloperFeatureOrNull(ai.getEffectiveUser().getId(), key);
    //                    if (feature != null && feature.getBooleanValue()) {
    //                        log.debug("Feature {} is set, adding trailer and username to
    // response", key);
    //                        msg += " " + trailer + ai.getEffectiveUser().getName();
    //                    } else
    //                        log.debug("Feature {} is not set, omitting trailer and username",
    // key);
    //
