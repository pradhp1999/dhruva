...
import com.cisco.dhruva.sip.stack.DsLibs.DsSipLlApi.DsSipClientTransactionInterface;
import com.cisco.dhruva.sip.stack.DsLibs.DsSipLlApi.DsSipClientTransportInfo;
import com.cisco.dhruva.sip.stack.DsLibs.DsSipLlApi.DsSipServerTransaction;
import com.cisco.dhruva.sip.stack.DsLibs.DsSipLlApi.DsSipServerTransactionInterface;
...
public class DsProxyTransaction extends DsProxyStatelessTransaction {
  ...
  private static final Trace Log = Trace.getTrace(DsProxyTransaction.class.getName());
  private static final Trace perfProxyTransactionLog = Trace.getTrace("perf.ProxyTransaction");
  ...
  public synchronized void init(
      ...
      DsSipRequest request)
      throws DsInternalProxyErrorException {
    if (Log.on && Log.isTraceEnabled()) Log.trace("Entering init()");
    ...
    if (branches != null) {
      ...
      if (Log.on && Log.isDebugEnabled()) Log.debug("No need to clear branches.  They're null!");
    }
    ...
    if (Log.on && Log.isTraceEnabled()) Log.trace("Leaving init()");
  }
  ...
  protected DsSipClientTransaction createClientTransaction(DsSipRequest request)
        ...
        new DsSipClientTransportInfo() {
          ...
          public Set getSupportedTransports(DsNetwork network) {
            ...
            if (lif != null) {
              ...
              if (Log.on && Log.isInfoEnabled()) Log.info("UDP transport added");
            }
            ...
            if (lif != null) {
              ...
              if (Log.on && Log.isInfoEnabled()) Log.info("TCP transport added");
            }
            ...
            if (lif != null) {
              ...
              if (Log.on && Log.isInfoEnabled()) Log.info("TLS transport added");
            }
    ...
    DsSipClientTransaction clientTrans =
  ...
  public synchronized void proxyTo(
    ...
    try {
      if (perfProxyTransactionLog.on) {
        perfProxyTransactionLog.debug("Entering DsProxyTransaction proxyTo()");
      }
      ...
      if (getStrayStatus() == STRAY_ACK || getStrayStatus() == STRAY_CANCEL) {
        ...
        if (perfProxyTransactionLog.on) {
          perfProxyTransactionLog.debug("Leaving DsProxyTransaction proxyTo()");
        }
  ...
  public synchronized void respond(DsSipResponse response) {
    ...
    try {
      ...
      if (responseClass != 2
        ...
        if (Log.on && Log.isDebugEnabled()) Log.debug("Response sent");
      } else if (Log.on && Log.isInfoEnabled())
    ...
    if (Log.on && Log.isTraceEnabled()) Log.trace("Leaving respond()");
  }
   ...
   * client and server transaction callbacks
   */
  ...
  protected synchronized void ackCallBack(DsSipServerTransaction trans, DsSipAckMessage ack) {
    ...
    if (Log.on && Log.isDebugEnabled()) Log.debug("Entering ackCallBack()");
  ...
  private boolean checkAckOnBranch(
    ...
    if (branchResponse != null
      ...
      if ((toTag == null && branchToTag == null) || (toTag != null && toTag.equals(branchToTag))) {
        ...
        try {
          if (Log.on && Log.isDebugEnabled()) Log.debug("propagating ACK to 200OK downstream...");
          ...
          if (branch.getState() != DsProxyClientTransaction.STATE_ACK_SENT) {
            ...
            branch.ack(ack);
            if (Log.on && Log.isDebugEnabled()) Log.debug("propagated ACK to 200OK downstream");
          } else {
  ...
  protected synchronized void cancelCallBack(
    ...
    if (Log.on && Log.isDebugEnabled()) Log.debug("Entering cancelCallBack()");
    try {
  ...
  protected synchronized void timeOut(DsSipServerTransaction trans) {
    ...
    if (trans != null
      ...
      if (Log.on && Log.isDebugEnabled()) Log.debug("Calling controller.onResponseTimeout()");
      controller.onResponseTimeOut(this, serverTrans);
    }
  }

  protected synchronized void timeOut(DsSipClientTransaction trans) {
    if (Log.on && Log.isDebugEnabled()) Log.debug("Entering timeOut()");
    DsProxyClientTransaction proxyClientTrans;
    ...
    if (clientState == DsProxyClientTransaction.STATE_PROV_RECVD) {
      if (Log.on && Log.isDebugEnabled()) Log.debug("cancelling ProxyClientTrans");
  ...
  protected synchronized void provisionalResponse(
    ...
    if (Log.on && Log.isDebugEnabled()) Log.debug("Entering provisionalResponse()");
    // look up in action table and do execute
    DsProxyClientTransaction clientTrans;
    ...
    if (Log.on && Log.isDebugEnabled()) Log.debug("Leaving provisionalResponse()");
  }

  protected synchronized void finalResponse(DsSipClientTransaction trans, DsSipResponse response) {
    if (Log.on && Log.isTraceEnabled()) Log.trace("Entering finalResponse()");
    ...
    if (Log.on && Log.isTraceEnabled()) Log.trace("Leaving finalResponse()");
  }
  ...
  protected class TransactionInterfaces
    ...
    public void cancel(DsSipServerTransaction trans, DsSipCancelMessage cancel) {
      if (Log.on && Log.isDebugEnabled()) Log.debug("cancel() call back called by the Low Level");
