...
/**
 * This class implements MessageForwarder interface defined by transport layer
 * This is singleton class and only one instance can be initiated
 * Receives the raw sip message and encapsulated in SipMessageBytes object having bindingInfo
 * It will invoke transaction layer asynchronously using completable future
 */
public class SipPacketProcessor implements MessageForwarder {
  ...
  /**
   * Implementation of processMessage provided by transport layer
   * Do not block here or do compute intensive work here, return ASAP
   * Callback registered with transport.
   * @param messageBytes Raw sip message bytes received from transport layer.
   *                     This is full message and message boundaries are taken care by transport
   *                     We are not expecting streams here
   * @param bindingInfo  Data structure that holds the connection information to be used by upper layers
   */
  @Override
  ...
  /**
   * Encapsulated the raw bytes in to Runnable implementation object holding the binding info and bytes
   * @param bytes
   * @param binfo
   * @return
   */
