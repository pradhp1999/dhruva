...
/**
 * Encapsulates parameters that can be passed to an outgoing
 * branch to modify default ProxyTransaction behavior
 */
public class DsProxyBranchParams implements DsProxyBranchParamsInterface {

  protected boolean doRecordRoute;
  protected int localProxyProtocol;
  ...
  /**
   * The constructor creates a DsProxyBranchInterface object
   * based on the configuration object passed
   */
  ...
  public DsByteString getRecordRouteUserParams()
  {
    ...
    /**
   * Specifies whether the proxy needs to insert itself into the
   * Record-Route
   ...
   * Specifies whether the proxy needs to insert itself into the
   * Record-Route
   * @param doRecordRoute if true, the proxy will insert itself
   * into Record-Route */
   ...
   * @return the address to proxy to, null if the default
   * forwarding logic is to be used */
   ...
   * @return the port to proxy to;
   * if -1 is returned, default port will be used */
 ...
 /**
   * Sets the IP address to send a SIP request to
   * NOTE: this will automatically disable local proxy settings
  ...
  /**
   * Sets the port to send a SIP request to
   * NOTE: this will automatically disable local proxy settings
  ...
  /**
   * Sets the protoco to use when sending a SIP request
   * NOTE: this will automatically disable local proxy settings
   * @param address protocol to use
   */
  public void setProxyToProtocol(int protocol) {
  ...
  /**
   * Removes the explicit destination address, thus
   * forcing the proxy to perform the default
   * forwarding logic
   */
  public void unsetLocalProxy() {
	  if (isLocalProxySet()) {
	    ...
	    localProxyPort = DsBindingInfo.REMOTE_PORT_UNSPECIFIED;
	    localProxyProtocol = DsSipTransportType.NONE;
  	}
  }
   ...
   * values <=0 or >=defaultTimeout have no effect
   */
  ...
  /**
   * @return ControllerConfig object at the time
   * this ProxyBranchParams was created
   */
   /*
