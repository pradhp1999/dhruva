...
public abstract class DsProxyResponseGenerator {
  ...
  public static DsSipResponse createRedirectResponse(ArrayList locations,
                                                     DsSipRequest request) throws Exception {
   ...
   */
  public static void sendRedirectResponse(ArrayList locations, DsSipRequest request,
                                          DsProxyTransaction trans) throws Exception {
  ...
  /**
   * This is a utility method that sends a redirect Response
   * depending on the number of contact elements in the contact header.  If there
   * are multiple contacts, then a 300 will be generated, otherwise a 302 will be
   * generate.
   * @param contactHeaders  <CODE>DsSipContactHeader</CODE> object containg the contact elemnts.
   * @param request The request original request that this response messeage is responding
   * to.
   * @param trans The proxy transaction that will be used to send the response.
   */
  public static void sendRedirectResponse(DsSipHeaderList contactHeaders,
  ...
  public static DsSipResponse createRedirectResponse(DsSipHeaderList contactHeaders,
                                                     /*DsSipContactHeader contactHeader,*/
                                                     DsSipRequest request) throws DsException {
    Log.debug("Entering createRedirectResponse()");
    ...
    if ((contactHeader != null) && (contactHeaders.size() > 1)) {
      response = new DsSipResponse(DsSipResponseCode.DS_RESPONSE_MULTIPLE_CHOICES,
                                   request, null, null);
    }
    else {
    //if one or less contact create redirect header with response
    // type-MOVED_TEMPORARILY ( #302)
      response = new DsSipResponse(DsSipResponseCode.DS_RESPONSE_MOVED_TEMPORARILY,
                                  request, null, null);
    }
  ...
  public static void sendServerInternalErrorResponse(DsSipRequest request, DsProxyTransaction trans)
    ...
    DsSipResponse response = new DsSipResponse(DsSipResponseCode.DS_RESPONSE_INTERNAL_SERVER_ERROR,
                                               request, null, null);
    response.setApplicationReason(DsMessageLoggingInterface.REASON_AUTO);
    DsSipToHeader toHeader = response.getToHeaderValidate();
    if (toHeader.getTag() == null) {
  ...
  public static DsSipResponse createNotFoundResponse(DsSipRequest request) throws DsException {
    ...
    DsSipResponse response = new DsSipResponse(DsSipResponseCode.DS_RESPONSE_NOT_FOUND,
                                               request, null, null);
    response.setApplicationReason(DsMessageLoggingInterface.REASON_AUTO);
    DsSipToHeader toHeader = response.getToHeaderValidate();
    if (toHeader.getTag() == null) {
  ...
  /**
   *	Utility function that sends 404 responses.  Note, you must use a stateful
   *  transaction to send the response.
   * @param request The request original request that this response messeage is responding
   * to.
   * @param trans The stateful proxy transaction that will be used to send the response.
   */
  public static void sendNotFoundResponse(DsSipRequest request, DsProxyTransaction trans) throws DsException {
  ...
  public static void sendByteBasedTryingResponse(DsProxyTransaction trans) {
    if (Log.on && Log.isTraceEnabled()) Log.trace("Entering sendByteBasedTryingResponse()");
    trans.respond(null);
    if (Log.on && Log.isTraceEnabled()) Log.trace("Leaving sendByteBasedTryingResponse()");
  }

  public static void sendTryingResponse(DsSipRequest request,
  ...
  public static void sendResponse(DsSipResponse response, DsProxyTransaction trans) {
    ...
    if (trans != null) {
      ...
      if (Log.on && Log.isEnabled(Level.WARN)) Log.warn("DsProxyTransaction was null!");
    }
   ...
   */
  public static DsSipResponse createResponse(int responseCode, DsSipRequest request) throws DsException {
  ...
  /**
   * {@link DsProxyResponseGenerator#addReasonHeader(DsSipResponse)} is called when CP goes maintenance mode<br/>
  ...
  public static void addMaintenanceReasonHeader(DsSipResponse response) {
      if(response == null || response.getStatusCode() != 503) {
          Log.warn("Maintenance mode Reason header added when CP generates 503 Service Unavaiable, but found : "+response);
          return;
      }
      DsSipReasonHeader reasonHeader = new DsSipReasonHeader();
      reasonHeader.setProtocol(DsSipConstants.BS_SIP);
      reasonHeader.setText(SAEventConstants.MAINTENANCE_MODE_REASON_PHRASE);
      reasonHeader.setCause(SAEventConstants.MAINTENANCE_MODE_REASON_CAUSE);
