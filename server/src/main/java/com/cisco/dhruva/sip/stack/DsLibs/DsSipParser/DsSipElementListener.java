...
public interface DsSipElementListener {
   ...
   */
  DsSipElementListener elementBegin(int contextId, int elementId)
   ...
   * @param isValid <code>true</code> if there were no exceptions while parsing; otherwise <code>
   *     false</code>
   * @throws DsSipParserListenerException when there is a problem with the data that was received
   */
  void elementFound(
  ...
  /**
   * The parser found a generic parameter. Only called when it is not a known parameter, for any
   * header or url, not just the current header.
   *
