...
import com.cisco.dhruva.sip.re.controllers.util.ParseProxyParamUtil;
import com.cisco.dhruva.sip.stack.DsLibs.DsSipLlApi.DsSipConnection;
import com.cisco.dhruva.sip.stack.DsLibs.DsSipLlApi.DsSipConnectionAssociations;
import com.cisco.dhruva.sip.stack.DsLibs.DsSipLlApi.DsSipDetServerLocator;
import com.cisco.dhruva.sip.stack.DsLibs.DsSipLlApi.DsSipServerLocator;
import com.cisco.dhruva.sip.stack.DsLibs.DsSipLlApi.DsSipSimpleResolver;
import com.cisco.dhruva.sip.stack.DsLibs.DsSipLlApi.DsSipTransactionManager;
import com.cisco.dhruva.sip.stack.DsLibs.DsSipObject.DsByteString;
import com.cisco.dhruva.sip.stack.DsLibs.DsSipObject.DsParameter;
...
public class DsProxyStatelessTransaction {
  ...
  private static final Transport Transports_UDP[] = {Transport.UDP, Transport.TCP, Transport.TLS};

  private static final Transport Transports_TCP[] = {Transport.TCP, Transport.UDP, Transport.TLS};

  private static final Transport Transports_TLS[] = {Transport.TLS, Transport.UDP, Transport.TCP};
  ...
  private static final Trace urlHeadersLog =
      Trace.getTrace(DsProxyStatelessTransaction.class.getName() + ".UrlHeaders");

  private static final Trace perfProxyStatelessTransactionLog =
      Trace.getTrace("perf.ProxyStatelessTransaction");
  ...
  public synchronized void init(
    ...
    if (Log.on && Log.isInfoEnabled()) Log.info("ProxyStatelessTransaction created");
  }
  ...
  public synchronized void proxyTo(
    ...
    if (perfProxyStatelessTransactionLog.on) {
      perfProxyStatelessTransactionLog.debug(
          "Entering DsProxyStatelessTransaction outer proxyTo()");
    }
  ...
  public synchronized void proxyTo(
    ...
    if (perfProxyStatelessTransactionLog.on) {
      perfProxyStatelessTransactionLog.debug("Entering DsProxyStatelessTransaction proxyTo()");
    }
    ...
    try {
      ...
      if (conid == null) {
        ...
        ret_connection =
          DsSipTransactionManager.getRequestConnection(request,
                                                       server_locator);
      ...
      */
      ...
      if (Log.on && Log.isEnabled(Level.ERROR)) Log.error("Got DsException in proxyTo()!", e);
      controller.onProxyFailure(
  ...
  protected synchronized void prepareRequest(
    ...
    try {
      ...
      if (Log.on && Log.isEnabled(Level.ERROR)) Log.error("Error getting Route header", e);
      route = null;
    }
    if (route == null) {
    ...
    if (processVia()) {
      ...
      Transport viaTransport = Transport.UDP;

      if (route == null) viaTransport = destTransport;
      else {
        if (route.getURI().isSipURL()) {
          DsSipURL routeURI = (DsSipURL) route.getURI();
          viaTransport =
              routeURI.hasTransport()
                  ? routeURI.getTransportParam()
                  : ParseProxyParamUtil.getNetworkTransport(request.getNetwork());
  ...
  protected DsViaListenInterface getPreferredListenIf(Transport protocol, String direction) {
  ...
  protected Transport setRequestDestination(
    ...
    if (destAddress != null) {
      try {
        ...
        if (Log.on && Log.isEnabled(Level.ERROR)) Log.error("Cannot set destination address!", e);
      }
    }
    ...
    if (destPort != 0) {
      ...
      if (Log.on && Log.isInfoEnabled()) Log.info("Request destination port is set to " + destPort);
    }
