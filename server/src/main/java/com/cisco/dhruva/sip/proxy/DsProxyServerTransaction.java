...
/**
 * This class represents the ServerTransaction on the receiving
 * side of ProxyTransaction. Note that more than one such transaction
 * may exist due to merged requests.
 */
...
public class DsProxyServerTransaction
{
    ...
    protected static Logger Log =
            DhruvaLoggerFactory.getLogger(DsProxyServerTransaction.class);


    protected DsProxyServerTransaction(DsProxyTransaction proxy,
                                       DsSipServerTransaction trans,
    ...
    public void respond(DsSipResponse response)
            throws DsDestinationUnreachableException
    {
        ...
        try
        {
            // Start collecting instrumentation data for Outgoing Response - User Agent
            DsRePerfManager.start(DsRePerfManager.OUTGOING_RESPONSE_UA);
            ...
            if (response != null)
            {
                ...
                if (DsControllerConfig.getCurrent().doRecordRoute())
            ...
            // Stop collecting instrumentation data for Outgoing Response - User Agent
            DsRePerfManager.stop(DsRePerfManager.OUTGOING_RESPONSE_UA);

            this.response = response;
            if (Log.on && Log.isDebugEnabled())
                Log.debug("Just sent a response");
            ...
            try
            {
                ...
                */
                ...
                if (pathHdrs != null)
                {
                    ...
                    while (pathHdr != null)
                    {
                        svcHeader = new DsSipServiceRouteHeader(pathHdr.getNameAddress(), pathHdr.getParameters());
                        svcHeaders.addFirst(svcHeader);
    ...
    /**
     * This is used to handle the special case with INVITE 200OK
     * retransmissions
     */
    protected void retransmit200()
            throws DsInvalidStateException, DsDestinationUnreachableException,
            DsException, IOException
    {
                ...
                DsProxyUtils.getResponseClass(response) == 2)
        {
            ...
            throw new DsInvalidStateException("Cannot retransmit in this state");
    }
    ...
    public void setRecordRouteInterface(DsSipMessage msg) throws DsException
    {
        ...
        DsRePerfManager.start(DsRePerfManager.INCOMING_RESPONSE_FW_RR);
        ...
        // Stop collecting instrumentation data for FW Trans - Set RR Interface
        DsRePerfManager.stop(DsRePerfManager.INCOMING_RESPONSE_FW_RR);
    ...
    private void setRRHelper(DsSipMessage msg, DsSipURL currentRRURL, boolean compress, DsTokenSipDictionary tokDic)
    {
        if (currentRRURL != null)
        {
            ...
            if (name != null)
            {
                ...
                if (Log.on && Log.isDebugEnabled()) Log.debug("Record Route URL to be modified : "+ currentRRURL);
                ...
                if (Log.on && Log.isDebugEnabled()) Log.debug("Outgoing network of the message for which record route has to be modified : " + network);
                ...
                if (Log.on && Log.isDebugEnabled()) Log.debug("Modified Record route URL to : " + currentRRURL);
            }
        }

    }

    private void setSRHelper(DsSipMessage msg, DsSipURL currentSRURL, boolean compress, DsTokenSipDictionary tokDic)
    {
        if (currentSRURL != null)
        {
            DsControllerConfig config = DsControllerConfig.getCurrent();
            String network = null;
            String name = config.checkPaths(currentSRURL.getUser(), currentSRURL.getHost(), currentSRURL.getPort(), currentSRURL.getTransportParam());
            if (name != null)
            {
                //todo optimize when get a chance
                ...
                if (user != null)
                {
                    ...
                    */
                    ...
                    if(user.startsWith(DsReConstants.PR_TOKEN))
                        ...
                        user = user.substring(0,user.length() - DsReConstants.PR_TOKEN1.length()) + DsReConstants.SR_TOKEN1;
                    else
                        user = user.replaceFirst(DsReConstants.PR_TOKEN2,DsReConstants.SR_TOKEN2);

                    currentSRURL.setUser(DsSipURL.getEscapedString(new DsByteString(user), DsSipURL.USER_ESCAPE_BYTES));
                }
                else
                {
                    network = msg.getNetwork().getName();
                ...
                DsSipServiceRouteHeader serviceRouteInterfaceHeader = config.getServiceRouteInterface(network);
