...
public class DsProxyErrorAggregator {
    ...
    private void sipRequestCheck() {
        if(request == null) {
            throw new IllegalStateException("DsSipRequest can't be NULL. Don't call any method without setting request.");
        }
    }
    ...
    public void onXclRoutingFailure(DsSipResponse response) {
        ...
        /**
         * xcl failure is called when CP can't find any routeGroup, connection refused & retry of next server group failure<br/>
         * CAEvent should be logged only when there is no routeGroup. Other failure logging is done wherever its required.<br/>
         * (ie) When connection refused & retry failure happens CA logging is done at DsProxyController#onProxyFailure()  
         * 
         * Refer dsnrs_route.xcl for more details
         */
    ...
    private DsProxyError getProxyError(Throwable throwable, DsBindingInfo bindingInfo, DsSipResponse response, int errorCode) {
    ...
    private void notifyError(DsProxyError proxyError, ErrorType errorType) {
        ...
        DsProxyErrorDetail errorDetail = new DsProxyErrorDetail(request, sipSession, errorType, proxyError);
