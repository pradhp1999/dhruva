...
public class DsMappedResponseCreator {
    ...
    private static String configFile = DsConfigManager.getProperty(DsConfigManager.PROP_ERROR_MAPPED_RESPONSE_CONFIG_FILE,
            DsConfigManager.PROP_ERROR_MAPPED_RESPONSE_CONFIG_FILE_PATH_DEFAULT);

    private static DsByteString X_CISCO_INTERNAL_REASON_HEADER = new DsByteString("X-Cisco-Internal-Reason");

    private static Trace Log = Trace.getTrace(DsMappedResponseCreator.class.getName());
    ...
    public DsSipResponse createresponse(String networkName, List<DsProxyError> proxyErrorList, DsSipResponse response) {
        ...
        try {
            ...
            if(proxyErrorList == null || proxyErrorList.isEmpty() || response.getStatusCode() < 300) {
                if (Log.on && Log.isInfoEnabled()) {
                    if(proxyErrorList == null || proxyErrorList.isEmpty()) {
                        ...
                        Log.info("skipping error mapped response as remote status code is " + response.getStatusCode());
                    }
                }
            ...
            if(!errorMappingTable.isMappingAvailableForNetwork(networkName)) {
                if (Log.on && Log.isInfoEnabled()) {
                    Log.info("skipping error mapped response as there is no error mapping for the network " + networkName);
                }
            ...
            do {
                ...
                for(int i = 1; i < proxyErrorListTemp.size(); i++) {
                    if(getProxyErrorPriority(proxyErrorListTemp.get(i)) < getProxyErrorPriority(proxyError)) {
                        proxyError = proxyErrorListTemp.get(i);
                    }
                }

                if(proxyError.getErrorCode() == DsProxyErrorCode.ERROR_SIP_FAILURE_RESPONSE) {
                    errorMapping = errorMappingTable.getResponseMappingInfo(networkName, ((DsProxyFailureResponse)proxyError).getSipResponse().getStatusCode());
                } else {
                    errorMapping = errorMappingTable.getResponseMappingInfo(networkName, proxyError.getErrorCode());
                }
                ...
                if(errorMapping == null) {
                    ...
                    if(proxyError.getErrorCode() == DsProxyErrorCode.ERROR_SIP_FAILURE_RESPONSE) {
                        log += "[remote response status: " + ((DsProxyFailureResponse)proxyError).getSipResponse().getStatusCode() + "]";
                    }

                    Log.info(log);
                }

                //remove this error from the list as it has no mapping configured. Try to find a mapping for the other errors in the list.
            ...
            if(errorMapping != null) {
                ...
                Log.info("skipping error mapped response as no mapping configured for the error(s) encountered");
            }
        } catch (Throwable t) {
    ...
    private void applyErrorMapping(DsSipResponse response, DsErrorMapping errorMapping, DsProxyError proxyError) throws Exception {
        ...
        DsSipUnknownHeader xCiscoInternalHeader = (DsSipUnknownHeader) response.getHeaderValidate(X_CISCO_INTERNAL_REASON_HEADER);
        ...
        if(proxyError.getErrorCode() == DsProxyErrorCode.ERROR_SIP_FAILURE_RESPONSE) {
            ...
            if(addXCiscoInternalReasonHeader) {
                ...
                xCiscoInternalHeader = (DsSipUnknownHeader) remoteResponse.getHeaderValidate(X_CISCO_INTERNAL_REASON_HEADER);
            }


            //info logging
            logMappedError.append(", remote response status: ").append(remoteResponse.getStatusCode()).append(", remote response reason phrase: ").append(remoteResponse.getReasonPhrase().toString());
            logMappedError.append(", remote response xCiscoInternalHeader: ").append(((xCiscoInternalHeader != null) ? xCiscoInternalHeader.toString() : "NOT PRESENT"));
        } else if(proxyError.getErrorCode() == DsProxyErrorCode.ERROR_XCL_ROUTING_FAIlURE) {

            DsSipResponse locallyGenratedResponse = ((DsXclRoutingFailureException)proxyError).getSipResponse();

            //use the internally generated response status code as mappedStatusCode, if mappedStatusCode is not configured
            if(mappedStatusCode == 0) {
                mappedStatusCode = locallyGenratedResponse.getStatusCode();
            }

            //use the internally generated response status code as mappedReasonPhrase, if mappedReasonPhrase is not configured
            if(mappedReasonPhrase == null) {
            ...
            //info logging
            logMappedError.append(", response status: ").append(locallyGenratedResponse.getStatusCode()).append(", response reason prase: ").append(locallyGenratedResponse.getReasonPhrase().toString());
        }
        ...
        if(addXCiscoInternalReasonHeader) {
            ...
            if(xCiscoInternalHeader == null) {
                xCiscoInternalHeader = createReasonHeader(xCiscoInternalHeaderCause, xCiscoInternalHeaderReason, xCiscoInternalHeaderSource);
            }

            response.addHeader(xCiscoInternalHeader);
        }

        Log.info("created mapped response[" + response.maskAndWrapSIPMessageToSingleLineOutput() + "] for error[" + logMappedError + "]");
    }
    ...
    private DsSipUnknownHeader createReasonHeader(int xCiscoInternalCause, String xCiscoInternalReason, String xCiscoInternalSource) {

        String value = "cause=" + xCiscoInternalCause + ";reason=" + xCiscoInternalReason + ";source=" + xCiscoInternalSource;
        DsSipUnknownHeader reasonHeader = new DsSipUnknownHeader(X_CISCO_INTERNAL_REASON_HEADER, new DsByteString(value));
        return reasonHeader;
    }
