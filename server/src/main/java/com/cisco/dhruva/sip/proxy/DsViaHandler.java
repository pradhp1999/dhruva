...
/**
 * An auxilary singleton class used to do Via header encryption/decsryption
 * and loop detection
 * Note that init() MUST be called before getInstance()
 */
...
public class DsViaHandler {
  ...
  private DsViaHandler(DsSipTransportLayer transportLayer)
    ...
    if (transportLayer == null)
      throw new NullPointerException("TransportLayer must not be null");
  ...
  /**
   * Takes an unencrypted Via header and encrypts it using
   * certain encryption algorithm
   * The encrypted <b>via</b> will contain hidden parameter
   * @param via Via header to encrypt. After the method returns,
   * the host-port is encrypted and the header contains hidden
   * parameter
   */
  ...
  /**
   * Takes an encrypted Via header and decrypts it
   * hidden parameter is stripped off
   * @param via encrypted Via header; after the methods is returned,
   * if the encryption was done by this proxy, the host-port part
   * will be decrypted and hidden parameter is removed; otherwise,
   * the via will stay unchanged
   * @return <b>true</b> if descyption was successful or not
   * encrypted, <b>false</b>
   * otherwise
   */
  protected boolean decryptVia(DsSipViaHeader via) {
    ...
    if (via.isHiddenPresent()) {
      ...
      try {
        ...
        if (colonIndex >= 0) {
          try {
            via.setPort(Integer.parseInt(DsByteString.toString(hostPort.substring(colonIndex + 1))));
          }
          catch (Exception e) {
  ...
  protected boolean isLocalInterface(DsByteString host, int port, int transport) {
    return transportLayer.isListenInterface(host.toString(), port, transport);
  }


  public static void init(DsSipTransportLayer transport)
