...
import static com.cisco.dhruva.sip.stack.DsLibs.DsSipObject.DsSipResponseCode.MAX_RESPONSE_CODE;
import static com.cisco.dhruva.sip.stack.DsLibs.DsSipObject.DsSipResponseCode.MIN_RESPONSE_CODE;
...
public class DsErrorMapping {
      ...
      for (String statusCode : statusCodes) {
        ...
        try {
          if (statusCodeRange.length == 1) {
            ...
            this.remoteStatusCodeRangeList.add(
                new RemoteStatusCodeRange(configuredStatusCode, configuredStatusCode));
            ...
            this.remoteStatusCodeRangeList.add(
                new RemoteStatusCodeRange(configuredStatusCodeMin, configuredStatusCodeMax));
          }
        } catch (Exception e) {
          Log.info(
              "Ignoring the exception ["
                  + e.getLocalizedMessage()
                  + "], while parsing configured remoteStatusCode");
        }
      }
    }
  ...
  public void setMappedStatusCode(int mappedStatusCode) throws Exception {
    ...
    if (mappedStatusCode >= MIN_RESPONSE_CODE && mappedStatusCode <= MAX_RESPONSE_CODE) {
      ...
      throw new Exception(
          "mappedStatusCode should lie between " + MIN_RESPONSE_CODE + " and " + MAX_RESPONSE_CODE);
    }
  }
