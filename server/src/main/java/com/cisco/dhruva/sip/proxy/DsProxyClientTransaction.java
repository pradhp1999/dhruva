...
/** This is a wrapper class for the ClientTransaction, which adds
 * some additional data members needed by the proxy and only
 * exposes the functionality needed by the Controller */

public class DsProxyClientTransaction {

  public static final String NL = System.getProperty("line.separator");

  protected final static int STATE_REQUEST_SENT = 0;
  protected final static int STATE_PROV_RECVD = 1;
  protected final static int STATE_FINAL_RECVD = 2;
  protected final static int STATE_ACK_SENT = 3;
  protected final static int STATE_CANCEL_SENT = 4;
  protected final static int STATE_FINAL_RETRANSMISSION_RECVD = 5;
  ...
  protected DsProxyClientTransaction(DsProxyTransaction proxy,
    ...
    if (Log.on && Log.isDebugEnabled()) Log.debug("ProxyClientTransaction created to " + request.getURI());
  ...
  /**
   * @return <b>true</b> if this is an INVITE transaction,
   * <b>false</b> otherwise
   */
   ...
   * sends an ACK
   */
  protected void ack() throws DsException, IOException, UnknownHostException,
    DsInvalidStateException {
   ...
   */
  protected void ack(DsSipAckMessage ack) throws DsException,
    IOException, UnknownHostException, DsInvalidStateException {
  ...
  protected void gotResponse(DsSipResponse resp) {
    Log.trace("Entering gotResponse()");
  ...
  protected boolean removeTimeout() {
    Log.trace("Entering removeTimeout()");
  ...
  /**
   * @return the last provisional or the final response
   * received by this transaction.
   ...
   * NOTE: modifying this response will have unpredictable consequences
