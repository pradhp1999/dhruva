...
public abstract class AbstractAppSession implements AppInterface {
  ...
  public void handleRequest(IDhruvaMessage request) {
    logger.debug("handleIncomingRequest {}" + request.getSessionId());
    if (appEngine == null) {
    ...
    try {
      ...
      throw new DhruvaException("exception {} while handling request {}" + e.getMessage() + request.getSessionId());
    }
  }

  public void handleResponse(IDhruvaMessage response) {

    logger.debug("onResponse: invoking message handler {}" + response.getSessionId());
  ...
  IDhruvaMessage createDummyResponse(IDhruvaMessage request) throws Exception {
    DsSipResponse resp =
        DsProxyResponseGenerator.createResponse(
            DsSipResponseCode.DS_RESPONSE_OK, (DsSipRequest) request.getMessageBody().getPayloadData());
