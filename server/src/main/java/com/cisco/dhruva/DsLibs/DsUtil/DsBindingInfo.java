  ...
  public String toString() {
    ...
    remote_info =
        " remote[[ port = "
            + m_RemotePort
            + " ("
            + (getRemoteAddressStr() != null ? m_RemoteAddressStr : "?")
            + ") ]]";
      ...
      local_info =
          " local[[ port = "
              ...
              + ((DsString.getHostAddress(m_LocalAddress) != null)
                  ? DsString.getHostAddress(m_LocalAddress)
                  : "?")
              + ")]]";
    } else {
    ...
    String nw = getNetwork() == null ? "null" : ("" + getNetwork().isTSIPEnabled());
    return transport
        + local_info
        + remote_info
        + connId
        + m_strConnectionId
        + ", network = "
        + DsNetwork.getNetwork(m_Network)
        + ", TSIP = "
        + nw;
  }
      ...
      try {
        ...
        if (DsLog4j.resolvCat.isEnabled(Level.WARN)) {
          DsLog4j.resolvCat.warn(
              "Exception while resolving the remote hostname in the Binding Info", t);
        }
