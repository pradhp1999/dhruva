...
/**
 * <p>This interface defines all the methods needed to get the actual next hop
 * destination and update the status of the attempt.  This interface was created
 * so that applications wouldn't have access to the <code>ServerGroupElement</code>
 * methods that directly modify the status, statistics, and information of that
 * object.
 */
public interface ServerInterface
{
    ...
    /**
     * Updates the state of this object when a failover occurs that is caused by something other
     * than receiving a Failover Response or an ICMP Error.
     */
    public void onFailover(String failureReason);

    /**
     * Updates the state of this object when a failover occurs that is caused by
     * receiving a Failover Response.
     * @param header the Retry-After header from the failover response.
     * If the header is <code>null</code>, the failure response default retry after value is used.
     */
